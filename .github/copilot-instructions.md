...
**3. Schema Validation**
```bash
# datasets/schema/insight.schema.json (v0.2) is regenerated by hand from
# cognitod::schema::Insight -- there is no live-fetch script or /insights/schema
# route (removed as YAGNI; scripts/fetch_insight_schema.py never existed).
# Update the schema file directly when the Insight struct changes.
...
The pipeline recognizes these incident classes (`cognitod::schema::InsightReason`, v0.2 --
see `datasets/schema/insight.schema.json`):
- **cpu_spin**: CPU >95%, runq elevated, hot pid dominates
- **noisy_neighbor**: another pod's CPU share dominates the victim's stall (formerly
  `high_cpu_contention` at the attribution layer, merged in v0.2)
- **io_saturation**: Disk util >90%, latency spikes, request queue elevated
- **fork_storm**: Forks/sec spike, short-lived shells
- **short_job_churn**: Thousands of <1s jobs, scheduler churn (formerly `short_job_flood`,
  renamed in v0.2 to match the attribution-layer name)
- **oom_risk**: RSS rising, swapin activity, cgroup near limit
- **runaway_tree**: Child process fan-out, tree growth
- **cpu_throttled**: cgroup `cpu.max` quota throttling, not a neighbour
- **disk_pressure**: ephemeral-storage eviction path
- **network_saturation**: retransmits / connection exhaustion
- **deployment_regression**: a rollout raised resource usage or regressed
...
- Dataset schema: `datasets/schema/insight.schema.json` (v0.2, hand-regenerated from `cognitod::schema::Insight`) and `datasets/schema/episode.schema.json` (v0.1, Incident Lab episode format)
- Dataset examples: `datasets/examples/v0.1-legacy/incident_insights.jsonl` (superseded shape, kept for history) and `datasets/episodes/golden/` (current episode fixtures)
