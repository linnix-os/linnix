...
### Added
- **`linnix-cli mcp serve`**: Linnix as an MCP server, so Claude Code, Codex or any other MCP client can query what cognitod observed instead of guessing about the running host. Five tools — `linnix_system_health`, `linnix_investigate_contention`, `linnix_explain_process`, `linnix_recent_incidents`, `linnix_explain_incident` — each taking a `detail` argument (`summary` | `evidence` | `raw`) so a caller pays for depth only once it has decided the host matters. Nothing is analysed in the MCP layer: contention answers come from `investigate::summarise` and incident answers from the daemon's own stored rendering, so an MCP answer and `linnix-cli investigate` cannot drift apart. No `confidence` field is emitted, deliberately — what the daemon establishes is contention attribution, not proven causality, and a float a consuming model would read as calibrated is worse than the evidence on its own. A daemon that is unreachable, unauthenticated, or running without the store behind a route each say so distinctly, because "no incidents" and "this daemon cannot answer that" lead an agent to opposite conclusions. See `docs/MCP.md`.
- **`linnix-cli investigate` no longer ranks offenders it cannot compare.** A window whose every row predates per-offender stall splitting leaves all offenders at zero attributed stall, so `summarise` falls back to sorting by name for reproducibility — and the renderer then labelled the alphabetically-first one `Likely offender` while admitting on the same line that its share was unrecorded. That is the same mistake as rendering an unknown share as 0%, which this renderer already refuses to make, pointed the other way. One such contender is enough to make the whole ordering unsound — it may outweigh every measured one — so a window containing any of them is now split into measured and unmeasured groups with no offender named, rather than only the all-legacy case.
- **`--url` is now a global flag on `linnix-cli`**, accepted on either side of a subcommand. It was previously declared on the top-level `Args` only, so `linnix-cli mcp serve --url ...` — the order anyone writes when configuring an MCP client, and the one every example uses — was rejected by clap. Existing `linnix-cli --url ... <subcommand>` invocations are unaffected.
...
- **Five-way feedback taxonomy**: `correct | wrong_culprit | wrong_reason | incomplete | what_fixed_it` replaces the old `useful | noise` (plus `wrong_root_cause`, which the `feedback` table's migration allowed but no code ever constructed). Splits "wrong" into what specifically failed -- the offending pod or the reason code -- which is the axis the Incident Lab scores. `linnix feedback <id> <rating>`, the Slack action buttons, and `/insights/{id}/feedback` and `/api/feedback` all take the new taxonomy; `linnix blame` now prints the correcting command under each insight. Existing rows are remapped on every daemon startup (`remap_legacy_feedback_labels` in `incidents.rs`, documented in `migrations/007_widen_feedback_taxonomy.sql`): `useful`→`correct`, `noise`→`incomplete`, `wrong_root_cause`→`wrong_culprit`.
