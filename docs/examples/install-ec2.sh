...
install_linnix_binaries() {
    ...
    if [ "$DEV_MODE" = true ]; then
        ...
        TAR_NAME="linnix-${ARCH_TAG}-unknown-linux-gnu.tar.gz"
        ...
        if ! wget -q "$DOWNLOAD_URL" 2>/dev/null; then
            log warn "Pre-built binaries not available, building from source..."
            cd /
            rm -rf "$TEMP_DIR"
            DEV_MODE=true
            install_dependencies
        ...
        if wget -q "${RELEASE_URL}/SHA256SUMS" 2>/dev/null; then
            if ! sha256sum --check --ignore-missing SHA256SUMS 2>/dev/null | grep -q "$TAR_NAME: OK"; then
            ...
            log warn "No checksums available, skipping verification"
        fi
